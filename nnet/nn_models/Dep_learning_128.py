...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.MLP_spe = nn.Linear(200, self.specific_dep_size)
    ...
    def forward(self, sentence, p_sentence,  pos_tags, lengths, target_idx_in, region_marks,
                ...
                sent_pred_lemmas_idx,  dep_tags,  dep_heads, targets, specific_dep_tags, test=False):
        ...
        for i in range(bf_e.size()[0]):
            for j in range(bf_e.size()[1]):
                concat_embeds[i, j] = bf_e[i, target_idx_in[i]]
        ...
        for predict_l, gold_l in zip(spe_dep_labels, specific_dep_tags.view(-1).data.numpy()):
        ...
        SPEDEPloss = loss_function(dep_tag_space_spe, specific_dep_tags.view(-1))
