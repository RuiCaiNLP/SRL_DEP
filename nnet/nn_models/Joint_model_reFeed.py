...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.hidden2tag_M_copy = nn.Linear(100, 200)
        ...
        self.num_layers = 2
        self.BiLSTM_SRL = nn.LSTM(input_size=lstm_hidden_dim * 2+200, hidden_size=lstm_hidden_dim, batch_first=True,
                                    bidirectional=True, num_layers=self.num_layers)
    ...
    def forward(self, sentence, p_sentence,  pos_tags, lengths, target_idx_in, region_marks,
        ...
        # b, roles
        #sub = torch.div(torch.add(local_roles_mask, -1), _BIG_NUMBER)
        ...
        loss = loss_function(tag_space, targets) + 0.1*loss_function(dep_tag_space, dep_tags.view(-1))
        return loss, tag_probs
