...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.num_layers = 1
        self.BiLSTM_SRL_0 = nn.LSTM(input_size=sent_embedding_dim_SRL + 2 * self.pos_size,
                                  hidden_size=lstm_hidden_dim, batch_first=True,
                                  bidirectional=True, num_layers=self.num_layers)

        init.orthogonal_(self.BiLSTM_SRL_0.all_weights[0][0])
        init.orthogonal_(self.BiLSTM_SRL_0.all_weights[0][1])
        init.orthogonal_(self.BiLSTM_SRL_0.all_weights[1][0])
        init.orthogonal_(self.BiLSTM_SRL_0.all_weights[1][1])

        self.num_layers = 3
        self.BiLSTM_SRL = nn.LSTM(input_size=lstm_hidden_dim * 2 + self.elmo_emb_size, hidden_size=lstm_hidden_dim, batch_first=True,
                                    bidirectional=True, num_layers=self.num_layers)
    ...
    def forward(self, sentence, p_sentence,  pos_tags, lengths, target_idx_in, region_marks,
        ...
        # hidden states [time_steps * batch_size * hidden_units]
        hidden_states, self.hidden_3 = self.BiLSTM_SRL_0(embeds_sort, self.hidden_3)
        # it seems that hidden states is already batch first, we don't need swap the dims
        # hidden_states = hidden_states.permute(1, 2, 0).contiguous().view(self.batch_size, -1, )
        hidden_states, lens = rnn.pad_packed_sequence(hidden_states, batch_first=True)
        # hidden_states = hidden_states.transpose(0, 1)
        ...
        # second_layer
        hidden_states = torch.cat((hidden_states, self.SRL_input_dropout(SRL_composer)))
