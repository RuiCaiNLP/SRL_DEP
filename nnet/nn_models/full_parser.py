...
class BiLSTMTagger(nn.Module):
    ...
    def forward(self, sentence, p_sentence,  pos_tags, lengths, target_idx_in, region_marks,
        ...
        sentence_cat = torch.cat((torch.from_numpy(np.zeros((self.batch_size, 1)).astype('int64')).to(device), sentence), 1)
        ...
        pos_tags_cat = torch.cat((torch.from_numpy(np.zeros((self.batch_size, 1)).astype('int64')).to(device), pos_tags), 1)
        pos_embeds = self.pos_embeddings(pos_tags_cat)
        region_marks_cat = torch.cat((torch.from_numpy(np.zeros((self.batch_size, 1)).astype('int64')).to(device), region_marks), 1)
