...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.hidden2tag = nn.Linear(2*lstm_hidden_dim, 2*lstm_hidden_dim)
        self.MLP = nn.Linear(2*lstm_hidden_dim, 4)
        ...
        self.num_layers = 4
        self.BiLSTM_SRL = nn.LSTM(input_size=sent_embedding_dim_SRL + 100, hidden_size=lstm_hidden_dim, batch_first=True,
                                    bidirectional=True, num_layers=self.num_layers)
    ...
    def forward(self, sentence, p_sentence,  pos_tags, lengths, target_idx_in, region_marks,
        ...
        for predict_l, gold_l in zip(dep_labels, specific_dep_relations.cpu().view(-1).data.numpy()):
        ...
        DEPloss = loss_function(dep_tag_space, specific_dep_relations.view(-1))
