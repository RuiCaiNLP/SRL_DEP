...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.num_layers = 2
        self.BiLSTM_SRL = nn.LSTM(input_size=lstm_hidden_dim * 2 *2, hidden_size=lstm_hidden_dim, batch_first=True,
                                    bidirectional=True, num_layers=self.num_layers)
    ...
    def forward(self, sentence, p_sentence,  pos_tags, lengths, target_idx_in, region_marks,
        ...
        head_features = torch.tensor(F.tanh(self.hidden2tag_M(bf_e)).data.numpy())
