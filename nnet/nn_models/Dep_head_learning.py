...
from nnet.nn_models import decoder
import os
import numpy as np
import torch
import math
...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.hidden2tag_M = nn.Linear(100, 100)
        self.hidden2tag_H = nn.Linear(100, 100)
    ...
    def __getExpr(self, head, modifier, i, j):
        output = self.MLP(F.tanh(head[i] + modifier[j]))
        return output

    def __evaluate(self, head, modifier):
        exprs = [[self.__getExpr(head, modifier, i, j)
                  for i in xrange(len(head))]
                 for j in xrange(len(modifier))]
    ...
    def forward(self, sentence, p_sentence,  pos_tags, lengths, target_idx_in, region_marks,
        ...
        e = 0
        DEPHeadloss = torch.tensor(0.0)
        errs = []
        bf_head = self.hidden2tag_H(bf_e)
        bf_modifier = self.hidden2tag_M(bf_e)
        for i in range(bf_e.size()[0]):
            score_matrix = []
            tensor_matrix = []
            for m in range(bf_modifier[i].size()[0]):
                score_array = []
                tensor_array = []
                for h in range(bf_head[i].size()[0]):
                    score = self.MLP(F.tanh(bf_head[i][h] + bf_modifier[i][m]))
                    tensor_array.append(score[0])
                    a = float(score[0].data.numpy())
                    score_array.append(a)
                score_matrix.append(score_array)
                tensor_matrix.append(tensor_array)
            golds = dep_heads[i]


            heads = np.argmax(np.array(score_matrix), axis=1)
            if i == 20:
                log(heads)
                log(golds)

            e = sum([1 for h, g in zip(heads, golds) if h != g-1])
            if e > 0:
                for m, (h, g) in enumerate(zip(heads, golds)):
                    if h != g-1 and g!=0:
                        DEPHeadloss += tensor_matrix[m][h] - tensor_matrix[m][g - 1]
        ...
        loss = SRLloss + DEPHeadloss
        return SRLloss, DEPHeadloss, DEPHeadloss, loss, SRLprobs, wrong_l_nums, all_l_nums, wrong_l_nums_spe, all_l_nums_spe
