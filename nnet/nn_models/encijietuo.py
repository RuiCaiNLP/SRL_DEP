...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.hidden2tag_0 = nn.Linear(2*lstm_hidden_dim, 2*lstm_hidden_dim)
        self.hidden2tag_1 = nn.Linear(2*lstm_hidden_dim, 2*lstm_hidden_dim)
        self.hidden2tag_2 = nn.Linear(2*lstm_hidden_dim, 2*lstm_hidden_dim)
        self.MLP = nn.Linear(4*lstm_hidden_dim, self.dep_size)

        self.tag2hidden = nn.Linear(self.dep_size, self.pos_size)

        self.hidden2tag_0_spe = nn.Linear(2 * lstm_hidden_dim, 2 * lstm_hidden_dim)
        self.hidden2tag_1_spe = nn.Linear(2 * lstm_hidden_dim, 2 * lstm_hidden_dim)
        self.hidden2tag_2_spe = nn.Linear(2 * lstm_hidden_dim, 2 * lstm_hidden_dim)
        ...
        self.num_layers = 1
        self.BiLSTM_0 = nn.LSTM(input_size=sent_embedding_dim, hidden_size=lstm_hidden_dim, batch_first=True,
        ...
        self.num_layers = 1
        self.BiLSTM_3 = nn.LSTM(input_size=lstm_hidden_dim * 2 + 1 * self.pos_size , hidden_size=lstm_hidden_dim, batch_first=True,
                                    bidirectional=True, num_layers=self.num_layers)

        init.orthogonal_(self.BiLSTM_3.all_weights[0][0])
        init.orthogonal_(self.BiLSTM_3.all_weights[0][1])
        init.orthogonal_(self.BiLSTM_3.all_weights[1][0])
        init.orthogonal_(self.BiLSTM_3.all_weights[1][1])
    ...
    def forward(self, sentence, p_sentence,  pos_tags, lengths, target_idx_in, region_marks,
        ...
        Label_composer = self.hidden2tag_0(hidden_states_0) + self.hidden2tag_1(hidden_states_1) + self.hidden2tag_2(hidden_states_2)
        ...
        Label_features = torch.cat((Label_composer, concat_embeds.transpose(0, 1)), 2)
        dep_tag_space = self.MLP(self.label_dropout(F.tanh(Label_features))).view(
            len(sentence[0]) * self.batch_size, -1)

        Link_composer = self.hidden2tag_0_spe(hidden_states_0) + self.hidden2tag_1_spe(hidden_states_1) + self.hidden2tag_2_spe(
            hidden_states_2)
        dep_tag_space_spe = self.MLP_spe(self.link_dropout(F.tanh(Link_composer))).view(
            len(sentence[0]) * self.batch_size, -1)
        ...
        hidden_states = torch.cat((hidden_states, h1), 2)
