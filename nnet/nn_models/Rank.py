...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.MLP = nn.Linear(2*lstm_hidden_dim, hps['pos_edim'] * 2)
        self.tag2hidden = nn.Parameter(torch.rand(self.dep_size, hps['pos_edim'] * 2))

        self.hidden2tag_spe = nn.Linear(2 * lstm_hidden_dim, 2 * lstm_hidden_dim)
        self.MLP_spe = nn.Linear(2 * lstm_hidden_dim, hps['pos_edim'] * 2)
        self.Link2hidden = nn.Parameter(torch.rand(4, hps['pos_edim'] * 2))
        ...
        self.num_layers = 4
        self.BiLSTM_SRL = nn.LSTM(input_size=self.elmo_emb_size + 4 * hps['pos_edim'] + sent_embedding_dim_SRL , hidden_size=lstm_hidden_dim, batch_first=True,
                                    bidirectional=True, num_layers=self.num_layers)
    ...
    def forward(self, sentence, p_sentence,  pos_tags, lengths, target_idx_in, region_marks,
        ...
        dep_tag_space = self.MLP(self.label_dropout(F.tanh(self.hidden2tag(Label_features)))).view(
            len(sentence[0]) * self.batch_size, -1).mm(self.tag2hidden.transpose(0, 1))
        ...
        dep_tag_space_spe = self.MLP_spe(self.link_dropout(F.tanh(self.hidden2tag_spe(Link_composer)))).view(
            len(sentence[0]) * self.batch_size, -1).mm(self.Link2hidden.transpose(0, 1))
        dep_tag_space_spe_use =self.MLP_spe(F.tanh(self.hidden2tag_spe(Link_composer))).view(
            len(sentence[0]) * self.batch_size, -1).mm(self.Link2hidden.transpose(0, 1))


        TagProbs_use = F.softmax(dep_tag_space_use, dim=1)
        LinkProbs_use = F.softmax(dep_tag_space_spe_use, dim=1)
        #construct SRL input
        TagProbs_noGrad = TagProbs_use.detach()
        LinkProbs_noGrad = LinkProbs_use.detach()
        h1 = F.relu(TagProbs_noGrad.mm(self.tag2hidden)).view(self.batch_size, len(sentence[0]), -1)
        h2 = F.relu(LinkProbs_noGrad.mm(self.Link2hidden)).view(self.batch_size, len(sentence[0]), -1)
        ...
        embeds_sort = rnn.pack_padded_sequence(embeds_sort, lengths_sort.cpu().numpy(), batch_first=True)
        # hidden states [time_steps * batch_size * hidden_units]
        hidden_states, self.hidden_3 = self.BiLSTM_SRL(embeds_sort, self.hidden_4)
        # it seems that hidden states is already batch first, we don't need swap the dims
        # hidden_states = hidden_states.permute(1, 2, 0).contiguous().view(self.batch_size, -1, )
        hidden_states, lens = rnn.pad_packed_sequence(hidden_states, batch_first=True)
        # hidden_states = hidden_states.transpose(0, 1)
