...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.num_layers = 4
        self.BiLSTM_SRL = nn.LSTM(input_size=1 * self.pos_size + sent_embedding_dim_SRL , hidden_size=lstm_hidden_dim, batch_first=True,
                                    bidirectional=True, num_layers=self.num_layers)
    ...
    def forward(self, sentence, p_sentence,  pos_tags, lengths, target_idx_in, region_marks,
        ...
        if test:
            h1 = self.dep_embeddings(dep_labels).view(self.batch_size, len(sentence[0]), -1)
        else:
            h1 = self.dep_embeddings(dep_tags).view(self.batch_size, len(sentence[0]), -1)
        ...
        SRL_hidden_states = torch.cat((embeds_SRL,  fixed_embeds, sent_pred_lemmas_embeds, pos_embeds, region_marks, h1), 2)
