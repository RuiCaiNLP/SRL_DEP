...
def test(model, train_set, test_set, converter, params_path):
    ...
    for gold_batch, batch in zip(train_set.batches(), test_set.batches()):
        ...
        sigf_file.write(str(int(right_NonNullPredict)) + ' ' +str(int(NonNullPredict)) +' ' + str(int(NonNullTruth))+'\n')
    ...
    Predicat_num = 1260
    P = (right_NonNullPredicts + Predicat_num*0.8618) / (NonNullPredicts + Predicat_num)
    R = (right_NonNullPredicts + Predicat_num*0.8618) / (NonNullTruths + Predicat_num)
    F1 = 2 * P * R / (P + R)
